...
<body>
    ...
    <div class="col-12 d-flex flex-direction-column flex-wrap justify-content-center my-4">
        <?php foreach ($voyages as $voyage) { ?>

            <div style="background-image:url('images/<?= $voyage['image'] ?>')" class='destination row col-3 mx-3 my-4 flex-direction-column align-items-center justify-content-center'>
                <div class="text-trip py-3">
                    <h1><?= $voyage['destination'] ?> </h1>
                    <?php if ($isLoggedIn == true) { ?>
                        <h2 style="color:red"><?= $voyage['prix'] * 0.8 ?> € </h2>
                    <?php } else { ?> <h2 style="color:red"><?= $voyage['prix'] ?> € </h2><?php } ?>

                    <h3><?= $voyage['duree'] ?> Jours</h3>
                    <h3>Pour <?= $voyage['personnes'] ?> voyageurs</h3>
                    <a href="voyage.php?destination=<?= $voyage['destination'] ?>">VOIR CE VOYAGE</a>
                </div>
